...
class YOLOLoss(nn.Module):    # 训练损失函数
...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio = 0.05, warmup_lr_ratio = 0.1, no_aug_iter_ratio = 0.05, step_num = 10):
    ...
    def multistep(lr, decay_rate, milestones, iters):
        n = iters // step_size
        if n not in milestones:
            return lr
        lr = lr * decay_rate ** milestones[n]
        return lr
    ...
    if lr_decay_type == "cos":
        ...
        decay_rate = 0.5    # 学习率下降率初步定为0.5
        step_size   = total_iters // step_num        # 修改学习率步长，即迭代多少次后修改学习率
        milestones = Counter([step_size // 3, step_size // 2, int(step_size / 1.5)])
        func = partial(multistep, lr, milestones, step_size)
    
    else:
        func = partial(hold_lr,lr)
