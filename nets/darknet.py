...
class SEWBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, shortcut=True, expansion=0.5, depthwise=False, act="silu"):
        ...
        # self.sn1 = LIFNode_with_negative_impulse(surrogate_function=surrogate.ATan(),detach_reset=True)
        self.sn1 = SignedIFNode(surrogate_function=surrogate.ATan(),detach_reset=True)
        #--------------------------------------------------#
        #   利用3x3卷积进行通道数的拓张。并且完成特征提取
        #--------------------------------------------------#
        self.conv2 = Conv(hidden_channels, out_channels, 3, stride=1, act=act)
        ...
        # self.sn2 = LIFNode_with_negative_impulse(surrogate_function=surrogate.ATan(),detach_reset=True)
        self.sn2 = SignedIFNode(surrogate_function=surrogate.ATan(),detach_reset=True)
